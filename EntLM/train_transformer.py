...
def main():
    ...
    if args.label_map_path is not None:
        ...
        import json
        with open(args.label_map_path, 'r', encoding='utf-8') as file:
            ori_label_token_map = json.load(file)
    else:
    ...
    def get_labels(predictions, references, tokens, emissions=None, viterbi_decoder=None):
        ...
        if use_crf:
            ...
            ori_tokens = [
                [tokenizer.convert_ids_to_tokens([t]) if isinstance(t, (int, np.integer)) else tokenizer.convert_ids_to_tokens(t)
